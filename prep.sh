...
#    - Copies `control`, `postinst`, and `prerm` files from the `debian` directory to `DEBIAN` directory for package installation.
#    - Copies the `d2d` executable from `bin` directory to `/usr/local/bin/`.
#    - Sets execute permissions for `control`, `postinst`, and `prerm` scripts in both `debian` and `DEBIAN` directories.
...
#    - Ensure that the necessary files (`control`, `postinst`, `prerm`, and `d2d`) are present in their respective directories before running this script.
...
#
# 5. **Error Handling**:
#    - The script assumes that all necessary files (`control`, `postinst`, `prerm`, and `d2d`) are present in their respective directories. Ensure their availability to avoid errors.
